...
CCS.JFrame.addGlobalFilters({
	...
	contextMenu: function(container){
		...
		delegates.each(function(delegate) {
                        if(!delegate.hasDataFilter('ContextMenu')) {
                                dbug.warn('you are using a deprecated JFrame filter (data-context-menu-actions) on %o, use the ContextMenu data-filter instead.', delegate);
                                delegate.addDataFilter('ContextMenu');
                        }
                });
	}
