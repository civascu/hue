...
/*
---
description: Adds keyboard functionality to HtmlTables 
provides: [Behavior.HtmlTableKeys]
requires: [Widgets/Behavior, More/HtmlTable.Sort, More/HtmlTable.Zebra, More/HtmlTable.Select]
script: Behavior.HtmlTableKeys.js
...
*/
...
Behavior.addGlobalPlugin('HtmlTable', 'HtmlTableKeys', function(element, events){
                var table = element;
		var selectable;
                var isSelectable = (table.hasClass('sortable') && !table.hasClass('noSelect')) || table.hasClass('selectable');
                var ht = table.retrieve('HtmlTable');
		...
		//Will not work without reference to JFrame
			...
			this.markForCleanup(function(){
				events.removeShortcuts(['Select Previous Row', 'Select Next Row']);
			}.bind(this));
		}
	}
